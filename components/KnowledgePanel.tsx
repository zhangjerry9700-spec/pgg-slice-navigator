...
export default function KnowledgePanel({ question }: KnowledgePanelProps) {
  if (!question) {
    return (
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg p-5">
        <div className="font-semibold text-[var(--text-primary)] mb-2">知识点卡片</div>
        <div className="text-sm text-[var(--text-secondary)]">开始答题后，这里会显示对应知识点。</div>
      </div>
    );
  }
  ...
  if (!card) {
    return (
      <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg p-5">
        <div className="font-semibold text-[var(--text-primary)] mb-2">知识点卡片</div>
        <div className="text-sm text-[var(--text-secondary)]">暂无相关知识点。</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-lg p-5">
      ...
      <div className="text-sm text-[var(--text-primary)] mb-3">{card.rule_summary}</div>
